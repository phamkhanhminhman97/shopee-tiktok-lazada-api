...
import { commonParameter, genURLwithSignature, getTimestampHoursAgo } from '../../common/helper';
...
export async function getOrderList(beforeHours: number, config: TiktokConfig) {
  ...
  try {
    while (hasMoreData) {
      ...
      const url = genURLwithSignature(TIKTOK_PATH.ORDER_LIST, commonParam, config);
 ...
 * @returns
 */
export async function getOrderDetail(orderNumber: string, config: TiktokConfig) {
  const timestamp = Math.floor(Date.now() / 1000);
  const commonParam = commonParameter(config, timestamp);
  const body = {
    order_id_list: [orderNumber],
  };
  const url = genURLwithSignature(TIKTOK_PATH.ORDER_DETAIL, commonParam, config);
  try {
